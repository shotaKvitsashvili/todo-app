<script lang="ts">
    ...
    let isExpired= parse(expireDate as string, 'MM/dd/yyyy - HH:mm', new Date()).getTime() < new Date().getTime()


    let todoStatus = classnames({
        'Finished': status === todoStatusEnums.completed && !isExpired,
        'Ongoing': status === todoStatusEnums.ongoing && !isExpired,
        'Expired': isExpired
    })
</script>
...
class="{
    classnames(
        ...
        `,
        {
            ...
            'bg-green-100': status === todoStatusEnums.completed,
            'bg-yellow-100': status === todoStatusEnums.ongoing,
    ...
    <div class="flex flex-col justify-between">
        <div class="flex items-center gap-2">
            ...
            <button
                ...
                class={status === todoStatusEnums.completed ? 'cursor-not-allowed' : ''}
            >
                <img
                    src={editable ? checkIcon : editIcon}
                    alt="edit"
                    class="w-4 h-10 transition-all hover:scale-[1.1]"
                />
            </button>
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
        ...
        <div class="inline-flex items-center">
            <label
                class="relative flex items-center p-3 rounded-full cursor-pointer {editable ? 'pointer-events-none' : ''}"
            >
                <input
                    type="checkbox"
                    onchange={(e: Event) => {
                        ...
                        updateTodo("statusUpdate", {
                                ...
                                finishedAt: checked? format(new Date(Date.now()), 'MM/dd/yyyy - HH:mm') : null
                            })
                    }}
                    class="before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-blue-gray-200 transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-gray-900 checked:bg-gray-900 checked:before:bg-gray-900 hover:before:opacity-10"
                    id="checkbox"
                    checked={status === todoStatusEnums.completed}
                />
                <span
...
<div class="flex justify-between items-center text-sm">
    ...
    {#if finishedAt}
        <span class="text-sm text-gray-600">
            Finished at: <span class="font-bold text-gray-900">{finishedAt}</span>
        </span>
    {/if}
    </div>
</div>
